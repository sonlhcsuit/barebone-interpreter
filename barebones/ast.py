...
class SyntaxTree(object):
    ...
    def read_token(self, type):
        try:
            ...
            if self.is_token(type):
                ...
                raise InterpreterError("Expected " + type + ", got " + str(token))
        except IndexError:
            raise InterpreterError("Expected " + type + ", got nothing")
    ...
    def read_keyword(self, types):
        ...
        if token[1] in types:
            ...
            raise InterpreterError("Expected one of " + str(types) + ", got " + str(token))
    ...
    def read_statement(self):
        if self.is_keyword([ "clear" ]):
            ...
            raise InterpreterError("Expected statement, got " + str(token))
